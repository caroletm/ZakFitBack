...
struct ObjectifController : RouteCollection {
    func boot(routes: any RoutesBuilder) throws {
        ...
        objectif.get(use: getAllObjectifs)
        objectif.post(use: createObjectif)
        
        objectif.group(":id") { objectif in
    ...
    func updateObjectifById(_ req: Request) async throws -> ObjectifDTO {
        ...
        guard let id = req.parameters.get("id", as: UUID.self),
              let objectif = try await Objectif.find(id, on: req.db)
        else {
            throw Abort(.notFound)
        }
